...
- **Renderer** — visual reviews render on the native render server: the same Unity avatar scene as the Web build, compiled as a Linux player that draws on the CPU with Mesa (no browser). The image downloads it from the `render-server-1` release of dcl-regenesislabs/wearable-validator (sha256-checked) and sets `RENDER_SERVER`, `RENDER_SERVER_BUILD` and `RENDER_SERVER_WORK_DIR`; unset `RENDER_SERVER` to render in Chromium instead. A 20-view item takes about 16 s on 4 vCPU where Chromium needs about a minute.
- **Isolation** — creator files are hostile input. The code checks run in a worker thread with a 60 s deadline, so a file that stalls them never blocks the server. Chromium runs inside its own sandbox where the container runtime allows the user namespaces it needs (`CHROMIUM_SANDBOX`, on by default; the startup self-test fails when it cannot start) and gets only `PATH`/`HOME`-style variables, never the server's tokens. It also runs as its own Linux user, `chrome` (`packages/server/chromium-user.sh`), and the server writes its run folders with umask 077, so even a Chromium exploit on a host where the sandbox cannot run reads neither the server's environment nor other creators' uploads. `packages/server/check-chromium-user.sh` proves it against a built image. The native render server runs as the same `chrome` user (`packages/server/render-server-user.sh` → `render-server.sh`) and gets only the variables it needs.
